...
public final class Role {
    ...
    @IntDef({Role.UNKNOWN, Role.ABBR, Role.ALERT, Role.ALERT_DIALOG, Role.APPLICATION, Role.ARTICLE, Role.AUDIO, Role.BANNER, Role.BLOCKQUOTE, Role.BUTTON, Role.CANVAS, Role.CAPTION, Role.CARET, Role.CELL, Role.CHECK_BOX, Role.CLIENT, Role.CODE, Role.COLOR_WELL, Role.COLUMN, Role.COLUMN_HEADER, Role.COMBO_BOX_GROUPING, Role.COMBO_BOX_MENU_BUTTON, Role.COMBO_BOX_SELECT, Role.COMPLEMENTARY, Role.COMMENT, Role.CONTENT_DELETION, Role.CONTENT_INSERTION, Role.CONTENT_INFO, Role.DATE, Role.DATE_TIME, Role.DEFINITION, Role.DESCRIPTION_LIST, Role.DESCRIPTION_LIST_DETAIL, Role.DESCRIPTION_LIST_TERM, Role.DESKTOP, Role.DETAILS, Role.DIALOG, Role.DIRECTORY, Role.DISCLOSURE_TRIANGLE, Role.DOC_ABSTRACT, Role.DOC_ACKNOWLEDGMENTS, Role.DOC_AFTERWORD, Role.DOC_APPENDIX, Role.DOC_BACK_LINK, Role.DOC_BIBLIO_ENTRY, Role.DOC_BIBLIOGRAPHY, Role.DOC_BIBLIO_REF, Role.DOC_CHAPTER, Role.DOC_COLOPHON, Role.DOC_CONCLUSION, Role.DOC_COVER, Role.DOC_CREDIT, Role.DOC_CREDITS, Role.DOC_DEDICATION, Role.DOC_ENDNOTE, Role.DOC_ENDNOTES, Role.DOC_EPIGRAPH, Role.DOC_EPILOGUE, Role.DOC_ERRATA, Role.DOC_EXAMPLE, Role.DOC_FOOTNOTE, Role.DOC_FOREWORD, Role.DOC_GLOSSARY, Role.DOC_GLOSS_REF, Role.DOC_INDEX, Role.DOC_INTRODUCTION, Role.DOC_NOTE_REF, Role.DOC_NOTICE, Role.DOC_PAGE_BREAK, Role.DOC_PAGE_FOOTER, Role.DOC_PAGE_HEADER, Role.DOC_PAGE_LIST, Role.DOC_PART, Role.DOC_PREFACE, Role.DOC_PROLOGUE, Role.DOC_PULLQUOTE, Role.DOC_QNA, Role.DOC_SUBTITLE, Role.DOC_TIP, Role.DOC_TOC, Role.DOCUMENT, Role.EMBEDDED_OBJECT, Role.EMPHASIS, Role.FEED, Role.FIGCAPTION, Role.FIGURE, Role.FOOTER, Role.FOOTER_AS_NON_LANDMARK, Role.FORM, Role.GENERIC_CONTAINER, Role.GRAPHICS_DOCUMENT, Role.GRAPHICS_OBJECT, Role.GRAPHICS_SYMBOL, Role.GRID, Role.GROUP, Role.HEADER, Role.HEADER_AS_NON_LANDMARK, Role.HEADING, Role.IFRAME, Role.IFRAME_PRESENTATIONAL, Role.IMAGE, Role.IME_CANDIDATE, Role.INLINE_TEXT_BOX, Role.INPUT_TIME, Role.KEYBOARD, Role.LABEL_TEXT, Role.LAYOUT_TABLE, Role.LAYOUT_TABLE_CELL, Role.LAYOUT_TABLE_ROW, Role.LEGEND, Role.LINE_BREAK, Role.LINK, Role.LIST, Role.LIST_BOX, Role.LIST_BOX_OPTION, Role.LIST_GRID, Role.LIST_ITEM, Role.LIST_MARKER, Role.LOG, Role.MAIN, Role.MARK, Role.MARQUEE, Role.MATH, Role.MATH_ML_FRACTION, Role.MATH_ML_IDENTIFIER, Role.MATH_ML_MATH, Role.MATH_ML_MULTISCRIPTS, Role.MATH_ML_NONE_SCRIPT, Role.MATH_ML_NUMBER, Role.MATH_ML_OPERATOR, Role.MATH_ML_OVER, Role.MATH_ML_PRESCRIPT_DELIMITER, Role.MATH_ML_ROOT, Role.MATH_ML_ROW, Role.MATH_ML_SQUARE_ROOT, Role.MATH_ML_STRING_LITERAL, Role.MATH_ML_SUB, Role.MATH_ML_SUB_SUP, Role.MATH_ML_SUP, Role.MATH_ML_TABLE, Role.MATH_ML_TABLE_CELL, Role.MATH_ML_TABLE_ROW, Role.MATH_ML_TEXT, Role.MATH_ML_UNDER, Role.MATH_ML_UNDER_OVER, Role.MENU, Role.MENU_BAR, Role.MENU_ITEM, Role.MENU_ITEM_CHECK_BOX, Role.MENU_ITEM_RADIO, Role.MENU_LIST_OPTION, Role.MENU_LIST_POPUP, Role.METER, Role.NAVIGATION, Role.NONE, Role.NOTE, Role.PANE, Role.PARAGRAPH, Role.PDF_ACTIONABLE_HIGHLIGHT, Role.PDF_ROOT, Role.PLUGIN_OBJECT, Role.POP_UP_BUTTON, Role.PORTAL, Role.PRE_DEPRECATED, Role.PROGRESS_INDICATOR, Role.RADIO_BUTTON, Role.RADIO_GROUP, Role.REGION, Role.ROOT_WEB_AREA, Role.ROW, Role.ROW_GROUP, Role.ROW_HEADER, Role.RUBY, Role.RUBY_ANNOTATION, Role.SCROLL_BAR, Role.SCROLL_VIEW, Role.SEARCH, Role.SEARCH_BOX, Role.SECTION, Role.SLIDER, Role.SPIN_BUTTON, Role.SPLITTER, Role.STATIC_TEXT, Role.STATUS, Role.STRONG, Role.SUBSCRIPT, Role.SUGGESTION, Role.SUPERSCRIPT, Role.SVG_ROOT, Role.SWITCH, Role.TAB, Role.TAB_LIST, Role.TAB_PANEL, Role.TABLE, Role.TABLE_HEADER_CONTAINER, Role.TERM, Role.TEXT_FIELD, Role.TEXT_FIELD_WITH_COMBO_BOX, Role.TIME, Role.TIMER, Role.TITLE_BAR, Role.TOGGLE_BUTTON, Role.TOOLBAR, Role.TOOLTIP, Role.TREE, Role.TREE_GRID, Role.TREE_ITEM, Role.VIDEO, Role.WEB_VIEW, Role.WINDOW})
