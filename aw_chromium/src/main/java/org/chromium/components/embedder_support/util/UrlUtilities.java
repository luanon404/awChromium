...
public class UrlUtilities {
    ...
    private static final List<String> SUPPORTED_SCHEMES = new ArrayList<String>(Arrays.asList(ContentUrlConstants.ABOUT_SCHEME, UrlConstants.DATA_SCHEME, UrlConstants.FILE_SCHEME, UrlConstants.HTTP_SCHEME, UrlConstants.HTTPS_SCHEME, UrlConstants.INLINE_SCHEME, UrlConstants.JAVASCRIPT_SCHEME));

    private static final List<String> DOWNLOADABLE_SCHEMES = new ArrayList<String>(Arrays.asList(UrlConstants.DATA_SCHEME, UrlConstants.BLOB_SCHEME, UrlConstants.FILE_SCHEME, UrlConstants.FILESYSTEM_SCHEME, UrlConstants.HTTP_SCHEME, UrlConstants.HTTPS_SCHEME));
    ...
    public static boolean sameDomainOrHost(String primaryUrl, String secondaryUrl, boolean includePrivateRegistries) {
        return UrlUtilitiesJni.get().sameDomainOrHost(primaryUrl, secondaryUrl, includePrivateRegistries);
    }
    ...
    public static String stripPath(String url) {
        ...
        return parsed.getScheme() + "://" + ((parsed.getHost() != null) ? parsed.getHost() : "") + ((parsed.getPort() != -1) ? (":" + parsed.getPort()) : "");
    }
    ...
    public static String extractPublisherFromPublisherUrl(GURL publisherUrl) {
        String publisher = UrlFormatter.formatUrlForDisplayOmitScheme(publisherUrl.getOrigin().getSpec());
    ...
    public static boolean hasIntentScheme(GURL url) {
        return url.getScheme().equals(UrlConstants.APP_INTENT_SCHEME) || url.getScheme().equals(UrlConstants.INTENT_SCHEME);
    }

    @NativeMethods
    public interface Natives {
        boolean sameDomainOrHost(String primaryUrl, String secondaryUrl, boolean includePrivateRegistries);
