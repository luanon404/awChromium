...
public class AndroidProtocolHandler {
    ...
    private static InputStream openByScheme(Uri uri) {
        try {
            if (uri.getScheme().equals(FILE_SCHEME)) {
                ...
                } else if (path.startsWith(AndroidProtocolHandlerJni.get().getAndroidResourcePath())) {
    ...
    private static Class<?> getClazz(String packageName, String assetType) throws ClassNotFoundException {
        return ContextUtils.getApplicationContext().getClassLoader().loadClass(packageName + ".R$" + assetType);
    }

    // file://android_res/ has no choice but to do name-based resource lookups
    @SuppressWarnings("DiscouragedApi")
    private static int getFieldId(String assetType, String assetName) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
    ...
    private static InputStream openResource(Uri uri) {
        ...
        if (!("/" + assetPath + "/").equals(AndroidProtocolHandlerJni.get().getAndroidResourcePath())) {
            Log.e(TAG, "Resource path does not start with " + AndroidProtocolHandlerJni.get().getAndroidResourcePath() + ": " + uri);
            return null;
        }
    ...
    private static InputStream openAsset(Uri uri) {
        ...
        String path = uri.getPath().replaceFirst(AndroidProtocolHandlerJni.get().getAndroidAssetPath(), "");
        try {
    ...
    public static String getMimeType(InputStream stream, GURL url) {
        ...
        try {
            ...
            } else if (uri.getScheme().equals(FILE_SCHEME) && path.startsWith(AndroidProtocolHandlerJni.get().getAndroidAssetPath())) {
    ...
    private static Uri verifyUrl(GURL url) {
        ...
        Uri uri = Uri.parse(url.getSpec()); // Never null. parse() doesn't actually parse or verify anything.
        String path = uri.getPath();
