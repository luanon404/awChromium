...
public class HostZoomMapImpl {
     ...
     */
    public static void setZoomLevel(WebContents webContents, double newZoomLevel, double adjustedZoomLevel) {
    ...
    public static HashMap<String, Double> getAllHostZoomLevels(BrowserContextHandle browserContextHandle) {
        SiteZoomInfo[] siteZoomInfoList = HostZoomMapImplJni.get().getAllHostZoomLevels(browserContextHandle);
    ...
    public static double getAdjustedZoomLevel(double zoomLevel, double desktopSiteZoomScale) {
        ...
        if (!ContentFeatureMap.isEnabled(ContentFeatureList.ACCESSIBILITY_PAGE_ZOOM) || !shouldAdjustForOSLevel()) {
            systemFontScale = 1;
        }
    ...
    public static boolean shouldAdjustForOSLevel() {
        return ContentFeatureMap.getInstance().getFieldTrialParamByFeatureAsBoolean(ContentFeatureList.ACCESSIBILITY_PAGE_ZOOM, ContentFeatureList.ACCESSIBILITY_PAGE_ZOOM_PARAM, true);
    }
     ...
     */
    public static double adjustZoomLevel(double zoomLevel, float systemFontScale, float desktopSiteZoomScale) {
        ...
        if (MathUtils.areFloatsEqual(systemFontScale, 1f) && MathUtils.areFloatsEqual(desktopSiteZoomScale, 1f) || !shouldAdjustForOSLevel()) {
            return zoomLevel;
        }
        ...
        double adjustedLevel = systemFontScale * Math.pow(TEXT_SIZE_MULTIPLIER_RATIO, zoomLevel) * desktopSiteZoomScale;
