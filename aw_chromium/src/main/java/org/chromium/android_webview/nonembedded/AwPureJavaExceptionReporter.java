...
/* package */ class AwPureJavaExceptionReporter extends PureJavaExceptionReporter {
    ...
    private static final String NONEMBEDDED_WEBVIEW_MINIDUMP_FILE_PREFIX = "nonembeded-webview-minidump-";
    ...
    protected void createReportFile() {
        ...
        if (mMinidumpFile != null) {
            File jsonLogFile = SystemWideCrashDirectories.createCrashJsonLogFile(mMinidumpFile.getName());
    ...
    protected File getCrashFilesDirectory() {
        if (!sCrashDirMade) {
            ...
            // removed from here.
            new File(SystemWideCrashDirectories.getOrCreateWebViewCrashDir(), CrashFileManager.CRASH_DUMP_DIR).mkdirs();
            sCrashDirMade = true;
        }
