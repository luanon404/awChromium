...
public class SimpleRecyclerViewMcp<T, VH> extends SimpleRecyclerViewMcpBase<T, VH, Void> {
     ...
     */
    public SimpleRecyclerViewMcp(ListModel<T> model, @Nullable ItemViewTypeCallback<T> itemViewTypeCallback, ViewBinder<T, VH> viewBinder) {
        super(itemViewTypeCallback, (holder, item, payload) -> viewBinder.onBindViewHolder(holder, item), model);
    }
}
