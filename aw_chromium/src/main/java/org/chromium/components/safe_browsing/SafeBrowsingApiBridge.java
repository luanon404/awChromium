...
public final class SafeBrowsingApiBridge {
    ...
    private static SafetyNetApiHandler initSafetyNetApiHandler() {
        try (TraceEvent t = TraceEvent.scoped("SafeBrowsingApiBridge.initSafetyNetApiHandler")) {
            ...
            return sSafetyNetApiHandler.init(new SafetyNetApiLookupDoneObserver()) ? sSafetyNetApiHandler : null;
        }
    }
    ...
    private static class SafetyNetApiLookupDoneObserver implements SafetyNetApiHandler.Observer {
        ...
        public void onUrlCheckDone(long callbackId, int resultStatus, String metadata, long checkDelta) {
            synchronized (sSafetyNetApiHandlerLock) {
                if (DEBUG) {
                    Log.i(TAG, "onUrlCheckDone resultStatus=" + resultStatus + ", metadata=" + metadata);
                }
                if (sSafetyNetApiUrlCheckTimeObserver != null) {
                    sSafetyNetApiUrlCheckTimeObserver.onUrlCheckTime(checkDelta);
                    TraceEvent.instant("FirstSafeBrowsingResponseFromSafetyNetApi", String.valueOf(checkDelta));
                    sSafetyNetApiUrlCheckTimeObserver = null;
                }
                SafeBrowsingApiBridgeJni.get().onUrlCheckDoneBySafetyNetApi(callbackId, resultStatus, metadata, checkDelta);
            }
        }
    }
    ...
    private static class SafeBrowsingApiLookupDoneObserver implements SafeBrowsingApiHandler.Observer {
        @Override
        public void onUrlCheckDone(long callbackId, @LookupResult int lookupResult, int threatType, int[] threatAttributes, int responseStatus, long checkDelta) {
            synchronized (sSafeBrowsingApiHandlerLock) {
                ...
                SafeBrowsingApiBridgeJni.get().onUrlCheckDoneBySafeBrowsingApi(callbackId, lookupResult, threatType, threatAttributes, responseStatus, checkDelta);
            }
        }
    }
     ...
     */
    @CalledByNative
    private static void startUriLookupBySafetyNetApi(long callbackId, String uri, int[] threatsOfInterest) {
        synchronized (sSafetyNetApiHandlerLock) {
            assert sSafetyNetApiHandlerInitCalled;
            assert sSafetyNetApiHandler != null;
            try (TraceEvent t = TraceEvent.scoped("SafeBrowsingApiBridge.startUriLookupBySafetyNetApi")) {
                if (DEBUG) {
     ...
     */
    @CalledByNative
    private static void startUriLookupBySafeBrowsingApi(long callbackId, String uri, int[] threatTypes, int protocol) {
        synchronized (sSafeBrowsingApiHandlerLock) {
            assert sSafeBrowsingApiHandler != null;
    ...
    @NativeMethods
    interface Natives {
        void onUrlCheckDoneBySafetyNetApi(long callbackId, int resultStatus, String metadata, long checkDelta);

        void onUrlCheckDoneBySafeBrowsingApi(long callbackId, int lookupResult, int threatType, int[] threatAttributes, int responseStatus, long checkDelta);
    }
}
