...
public class Utils {
     ...
     */
    public static void writeByteArrayToStream(@NonNull byte[] inputBytes, @NonNull OutputStream outputStream) {
        try {
     ...
     */
    @NonNull
    public static AssetFileDescriptor writeBytesIntoPipeAsync(@NonNull byte[] inputBytes, @NonNull ExecutorService executorService) throws IOException {
        ParcelFileDescriptor[] pipe = ParcelFileDescriptor.createPipe();
        ParcelFileDescriptor readSide = pipe[0];
        ParcelFileDescriptor writeSide = pipe[1];
        OutputStream outputStream = new ParcelFileDescriptor.AutoCloseOutputStream(writeSide);
        executorService.execute(() -> Utils.writeByteArrayToStream(inputBytes, outputStream));
     ...
     */
    public static int readNBytes(@NonNull InputStream inputStream, @NonNull byte[] b, int off, int len) throws IOException {
        int n = 0;
        while (n < len) {
     ...
     */
    @NonNull
    public static String readToString(@NonNull AssetFileDescriptor afd, int maxLength, boolean truncate) throws IOException, LengthLimitExceededException {
        try {
            ...
            if (afd.getLength() > maxLength) {
                if (truncate) {
                    ...
                    throw new LengthLimitExceededException("AssetFileDescriptor.getLength() should be" + " <= " + maxLength);
                }
            }
            ...
            try (ParcelFileDescriptor pfd = afd.getParcelFileDescriptor()) {
                ...
                if (Utils.readNBytes(inputStream, bytes, 0, lengthToRead) != lengthToRead) {
                    throw new IOException("Couldn't read " + lengthToRead + " bytes from the " + "AssetFileDescriptor");
                }
            }
