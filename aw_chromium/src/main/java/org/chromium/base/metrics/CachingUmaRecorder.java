...
/* package */ final class CachingUmaRecorder implements UmaRecorder {
    ...
    static class Histogram {
         ...
         */
        @IntDef({Type.BOOLEAN, Type.EXPONENTIAL, Type.LINEAR, Type.SPARSE,})
        ...
        Histogram(@Type int type, String name, int min, int max, int numBuckets) {
            assert type == Type.EXPONENTIAL || type == Type.LINEAR || (min == 0 && max == 0 && numBuckets == 0) : "Histogram type " + type + " must have no min/max/buckets set";
            mType = type;
            mName = name;
            mMin = min;
            mMax = max;
         ...
         */
        synchronized boolean addSample(@Type int type, String name, int sample, int min, int max, int numBuckets) {
            assert mType == type;
    ...
    private void flushUserActionsAlreadyLocked(List<UserAction> cache, int droppedUserActionCount) {
        ...
        Log.i(TAG, "Flushed %d user action samples, %d samples were dropped.", cache.size(), droppedUserActionCount);
    }
     ...
     */
    private void cacheOrRecordHistogramSample(@Histogram.Type int type, String name, int sample, int min, int max, int numBuckets) {
     ...
     */
    private boolean tryAppendOrRecordSample(@Histogram.Type int type, String name, int sample, int min, int max, int numBuckets) {
        mRwLock.readLock().lock();
        try {
            if (mDelegate != null) {
                recordHistogramSampleAlreadyLocked(type, name, sample, min, max, numBuckets);
     ...
     */
    @GuardedBy("mRwLock")
    private void cacheHistogramSampleAlreadyWriteLocked(@Histogram.Type int type, String name, int sample, int min, int max, int numBuckets) {
     ...
     */
    @GuardedBy("mRwLock")
    private void recordHistogramSampleAlreadyLocked(@Histogram.Type int type, String name, int sample, int min, int max, int numBuckets) {
        assert mRwLock.getReadHoldCount() > 0;
        assert !mRwLock.isWriteLockedByCurrentThread();
