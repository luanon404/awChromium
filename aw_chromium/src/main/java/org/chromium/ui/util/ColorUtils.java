...
public class ColorUtils {
    ...
    public static int getThemedAssetColor(int themeColor, boolean isIncognito) {
        if (ColorUtils.shouldUseLightForegroundOnBackground(themeColor) || isIncognito) {
            ...
            // Light theme.
            return ColorUtils.getColorWithOverlay(themeColor, Color.BLACK, THEMED_FOREGROUND_BLACK_FRACTION);
        }
    }
     ...
     */
    public static int getColorWithOverlay(int baseColor, int overlayColor, float overlayAlpha, boolean considerOpacity) {
        int red = (int) MathUtils.interpolate(Color.red(baseColor), Color.red(overlayColor), overlayAlpha);
        int green = (int) MathUtils.interpolate(Color.green(baseColor), Color.green(overlayColor), overlayAlpha);
        int blue = (int) MathUtils.interpolate(Color.blue(baseColor), Color.blue(overlayColor), overlayAlpha);
        if (considerOpacity) {
            int alpha = (int) MathUtils.interpolate(Color.alpha(baseColor), Color.alpha(overlayColor), overlayAlpha);
     ...
     */
    public static @ColorInt int setAlphaComponent(@ColorInt int color, @IntRange(from = 0L, to = 255L) int alpha) {
