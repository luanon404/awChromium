 ...
 */
@VisibleForTesting
class AwPolicyProvider(context: Context?) : AppRestrictionsProvider(context) {
    override fun notifySettingsAvailable(newAppRestrictions: Bundle) {
        val filteredRestrictions: Bundle?
        filteredRestrictions = Bundle()
        for (key in newAppRestrictions.keySet()) {
            if (!key.startsWith(POLICY_PREFIX)) continue
            filteredRestrictions.putSerializable(
                key.substring(POLICY_PREFIX.length),
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    newAppRestrictions.getSerializable(key, String::class.java)
                } else {
    ...
    companion object {
        ...
        const val POLICY_PREFIX = "com.android.browser:"
    }
}