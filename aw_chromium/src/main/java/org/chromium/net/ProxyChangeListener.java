...
public class ProxyChangeListener {
    ...
    private static class ProxyConfig {
        ...
        private static ProxyConfig fromProxyInfo(ProxyInfo proxyInfo) {
            ...
            return new ProxyConfig(host == null ? "" : host, proxyInfo.getPort(), Uri.EMPTY.equals(pacFileUrl) ? null : pacFileUrl.toString(), proxyInfo.getExclusionList());
        }

        @Override
        public String toString() {
            String possiblyRedactedHost = mHost.equals("localhost") || mHost.isEmpty() ? mHost : "<redacted>";
            return String.format(Locale.US, "ProxyConfig [mHost=\"%s\", mPort=%d, mPacUrl=%s]", possiblyRedactedHost, mPort, mPacUrl == null ? "null" : "\"<redacted>\"");
        }
    ...
    private void proxySettingsChanged(ProxyConfig cfg) {
        ...
        if (cfg != null) {
            ProxyChangeListenerJni.get().proxySettingsChangedTo(mNativePtr, ProxyChangeListener.this, cfg.mHost, cfg.mPort, cfg.mPacUrl, cfg.mExclusionList);
        } else {
    ...
    private ProxyConfig getProxyConfig(Intent intent) {
        ...
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && configFromConnectivityManager.mHost.equals("localhost") && configFromConnectivityManager.mPort == -1) {
            ProxyConfig configFromIntent = extractNewProxy(intent);
            Log.i(TAG, "configFromConnectivityManager = %s, configFromIntent = %s", configFromConnectivityManager, configFromIntent);
    ...
    private void registerBroadcastReceiver() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            ContextUtils.registerProtectedBroadcastReceiver(ContextUtils.getApplicationContext(), mProxyReceiver, filter);
        } else {
            if (!ContextUtils.isSdkSandboxProcess()) {
                ...
                ContextUtils.registerNonExportedBroadcastReceiver(ContextUtils.getApplicationContext(), mProxyReceiver, new IntentFilter());
            }
            ...
            ContextUtils.registerProtectedBroadcastReceiver(ContextUtils.getApplicationContext(), mRealProxyReceiver, filter);
        }
    }
    ...
    interface Natives {
        ...
        void proxySettingsChangedTo(long nativePtr, ProxyChangeListener caller, String host, int port, String pacUrl, String[] exclusionList);
