...
public class ChildProcessLauncher {
    ...
    public boolean start(final boolean setupConnection, final boolean queueIfNoFreeConnection) {
        ...
        try {
            ...
            ChildProcessConnection.ServiceCallback serviceCallback = new ChildProcessConnection.ServiceCallback() {
                @Override
            ...
            if (!allocateAndSetupConnection(serviceCallback, setupConnection, queueIfNoFreeConnection) && !queueIfNoFreeConnection) {
                return false;
            }
            return true;
        } finally {
    ...
    private boolean allocateAndSetupConnection(final ChildProcessConnection.ServiceCallback serviceCallback, final boolean setupConnection, final boolean queueIfNoFreeConnection) {
        assert mConnection == null;
        Bundle serviceBundle = new Bundle();
        mDelegate.onBeforeConnectionAllocated(serviceBundle);

        mConnection = mConnectionAllocator.allocate(ContextUtils.getApplicationContext(), serviceBundle, serviceCallback);
        if (mConnection == null) {
    ...
    private void setupConnection() {
        ChildProcessConnection.ZygoteInfoCallback zygoteInfoCallback = new ChildProcessConnection.ZygoteInfoCallback() {
            @Override
            public void onReceivedZygoteInfo(ChildProcessConnection connection, Bundle relroBundle) {
        ...
        mConnection.setupConnection(connectionBundle, getClientInterfaces(), connectionCallback, zygoteInfoCallback);
    }
