...
public class FontPreloadingWorkaround {
    ...
    public static void maybeInstallWorkaround(Context appContext) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O || Build.VERSION.SDK_INT >= Build.VERSION_CODES.P || !ContextUtils.isIsolatedProcess()) {
            return;
        }
        ...
        try {
            ...
            // Make the proxy.
            Object wrappedPackageManager = Proxy.newProxyInstance(packageManagerClassLoader, new Class[]{packageManagerInterface}, new PackageManagerWrapper(packageManager));
