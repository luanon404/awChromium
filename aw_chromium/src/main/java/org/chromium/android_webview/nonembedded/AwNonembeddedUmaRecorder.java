...
public class AwNonembeddedUmaRecorder implements UmaRecorder {
    ...
    public void recordUserAction(String name, long elapsedRealtimeMillis) {
        HistogramRecord record = HistogramRecord.newBuilder().setRecordType(RecordType.USER_ACTION).setHistogramName(name).setElapsedRealtimeMillis(elapsedRealtimeMillis).build();
    ...
    private void maybeBindServiceLocked() {
        ...
        mIsBound = ServiceHelper.bindService(appContext, intent, mServiceConnection, Context.BIND_AUTO_CREATE);
        if (!mIsBound) {
