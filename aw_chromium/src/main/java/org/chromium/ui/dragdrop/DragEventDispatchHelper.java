...
public class DragEventDispatchHelper implements OnDragListener {
    static final int[] ALL_DRAG_ACTIONS = new int[]{DragEvent.ACTION_DRAG_STARTED, DragEvent.ACTION_DRAG_LOCATION, DragEvent.ACTION_DROP, DragEvent.ACTION_DRAG_ENDED, DragEvent.ACTION_DRAG_ENTERED, DragEvent.ACTION_DRAG_EXITED};
    ...
    private final SparseBooleanArray mSupportedActionsMask = new SparseBooleanArray(ALL_DRAG_ACTIONS.length);
    ...
    public DragEventDispatchHelper(View sourceView, DragEventDispatchDestination destination) {
        ...
        for (int action : ALL_DRAG_ACTIONS) {
            ...
            boolean supported = (action != DragEvent.ACTION_DRAG_STARTED && action != DragEvent.ACTION_DRAG_ENDED);
    ...
    public boolean onDrag(View v, DragEvent event) {
        ...
        if (destinationView == null || !destinationView.isEnabled() || !destinationView.isAttachedToWindow() || !isActionSupported(event.getAction())) {
            return isDragStart;
        }
        ...
        if (event.getAction() == DragEvent.ACTION_DRAG_ENTERED || event.getAction() == DragEvent.ACTION_DRAG_EXITED || event.getAction() == DragEvent.ACTION_DRAG_ENDED) {
