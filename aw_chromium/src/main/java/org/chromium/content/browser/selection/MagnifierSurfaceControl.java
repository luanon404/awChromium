...
public class MagnifierSurfaceControl implements MagnifierWrapper {
    ...
    public MagnifierSurfaceControl(WebContentsImpl webContents, SelectionPopupControllerImpl.ReadbackViewCallback callback) {
    ...
    public void show(float x, float y) {
        ...
        if (mSurfaceControl != null) {
            ...
            MagnifierSurfaceControlJni.get().setReadbackOrigin(mNativeMagnifierSurfaceControl, x, readback_y);

            int[] viewOriginInSurface = new int[2];
            getView().getLocationInSurface(viewOriginInSurface);
            mTransaction.setPosition(mSurfaceControl, x + viewOriginInSurface[0], y + viewOriginInSurface[1]);
            mTransaction.apply();
        }
    }
    ...
    private void createNativeIfNeeded() {
        ...
        SurfaceControl.Transaction attachTransaction = attachedSurfaceControl.buildReparentTransaction(surfaceControl);
        ...
        mNativeMagnifierSurfaceControl = MagnifierSurfaceControlJni.get().create(mWebContents, surfaceControl, density, mWidthPx, mHeightPx, cornerRadius, zoom, scaleByDeviceFactor(TOP_SHADOW_HEIGHT_DP), scaleByDeviceFactor(BOTTOM_SHADOW_HEIGHT_DP), scaleByDeviceFactor(BOTTOM_SHADOW_WIDTH_REDUCTION_DP));
    ...
    @NativeMethods
    interface Natives {
        long create(WebContentsImpl webContents, SurfaceControl surfaceControl, float deviceScale, int width, int height, float cornerRadius, float zoom, int topShadowHeight, int bottomShadowHeight, int bottomShadowWidthReduction);
